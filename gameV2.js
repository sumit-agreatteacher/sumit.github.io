...
function makeCharacterSpeak(characterId, message) {
  ...
  if (bubble) {
    ...
    // Fallback: if the sidebar bubble isn't present (e.g., 'sumit' card not in sidebar),
    // write to the log so the tutorial still appears.
    // Also show a transient top-center toast for visibility.
    try {
      showToast && showToast(`${characterId}: ${message}`, 2500);
    } catch(e) {}
  }
  // Always log the line to the message window.

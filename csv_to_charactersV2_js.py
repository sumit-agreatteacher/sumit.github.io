...
# === Behavior ===
# - Default: GLOBAL (non-ESM) and *reassignable* outputs using `var`.
# - With --esm: ESM output using `const` + export.
# - --derive-globals: when using single-array mode, also emit phd/postdoc/coPI arrays.
...
def emit_single_array(rows, header, out_fp, esm: bool, derive_globals: bool):
    ...
    if derive_globals:
        ...
        out_fp.write(f"{decl} phdCandidates     = characters.filter(c => (String(c.type || \"\").toLowerCase()) === \"phd\" && !c.hired);\n")
        out_fp.write(f"{decl} postdocCandidates = characters.filter(c => (String(c.type || \"\").toLowerCase()) === \"postdoc\" && !c.hired);\n")
        out_fp.write(f"{decl} coPICandidates    = characters.filter(c => (String(c.type || \"\").toLowerCase()) === \"copi\" && !c.hired);\n")
    ...
    if esm:
        ...
        # In global mode, optionally mirror onto window (harmless and often useful)
        out_fp.write("\nif (typeof window !== 'undefined') { window.characters = characters;\n")
        if derive_globals:
            out_fp.write("  window.phdCandidates = phdCandidates;\n  window.postdocCandidates = postdocCandidates;\n  window.coPICandidates = coPICandidates;\n")
        out_fp.write("}\n")

def emit_grouped(rows, header, out_fp, esm: bool):
    decl = "const" if esm else "var"
    cats = {"characters": [], "phdCandidates": [], "postdocCandidates": [], "coPICandidates": []}
    for row in rows:
        t = str(row.get("type", "")).strip().lower()
        if t == "phd":
            cats["phdCandidates"].append(row)
        elif t == "postdoc":
            cats["postdocCandidates"].append(row)
        elif t == "copi":
            cats["coPICandidates"].append(row)
        else:
            cats["characters"].append(row)

    out_fp.write("// Generated by csv_to_characters_js.py (grouped-by-type mode)\n")

    def write_array(name, arr):
        out_fp.write(f"{decl} {name} = [\n")
    ...
    for name, arr in cats.items():
        write_array(name, arr)

    if esm:
        out_fp.write("export { characters, phdCandidates, postdocCandidates, coPICandidates };\n")
    else:
        out_fp.write("if (typeof window !== 'undefined') { window.characters = characters; window.phdCandidates = phdCandidates; window.postdocCandidates = postdocCandidates; window.coPICandidates = coPICandidates; }\n")

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--csv", default="charactersV2.csv", help="Input CSV path")
    ap.add_argument("--out", default="charactersV2.js", help="Output JS path")
    ...
    ap.add_argument("--esm", action="store_true", help="Emit ES module exports instead of global vars")
    ...
    with open(args.out, "w", encoding="utf-8", newline="\n") as out_fp:
        if args.group_by_type:
            ...
            emit_single_array(rows, header, out_fp, esm=args.esm, derive_globals=(not args.esm) or args.derive_globals)
